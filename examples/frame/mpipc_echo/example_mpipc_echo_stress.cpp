...
struct SocketAddressCmp {
    bool operator()(
        ...
        const PeerAddressPairT* const& _pa2) const
    {
...
struct Params {
    ...
    typedef std::map<const PeerAddressPairT*,
        uint32_t,
        SocketAddressCmp>
        SocketAddressMapT;
    string dbg_levels;
...
struct FirstMessage : Dynamic<FirstMessage, DynamicShared<frame::ipc::Message>> {
    ...
    template <class S>
    void solidSerializeV1(S& _s, frame::ipc::ConnectionContext const& _rctx)
    {
...
bool parseArguments(Params& _par, int argc, char* argv[])
{
    ...
    try {
        ...
        desc.add_options()("help,h", "List program options")("debug-levels,L", value<string>(&_par.dbg_levels)->default_value("view"), "Debug logging levels")("debug-modules,M", value<string>(&_par.dbg_modules), "Debug logging modules")("debug-address,A", value<string>(&_par.dbg_addr), "Debug server address (e.g. on linux use: nc -l 9999)")("debug-port,P", value<string>(&_par.dbg_port)->default_value("9999"), "Debug server port (e.g. on linux use: nc -l 9999)")("debug-console,C", value<bool>(&_par.dbg_console)->implicit_value(true)->default_value(false), "Debug console")("debug-unbuffered,S", value<bool>(&_par.dbg_buffered)->implicit_value(false)->default_value(true), "Debug unbuffered")("base-port,b", value<int>(&_par.baseport)->default_value(2000), "IPC Base/Listen port")("connect,c", value<vector<string>>(&_par.connectstringvec), "Peer to connect to: netid:YYY.YYY.YYY.YYY:port")("gateway,g", value<vector<string>>(&_par.gatewaystringvec), "Gateways to connect through: YYY.YYY.YYY.YYY:port")("repeat-count", value<uint32_t>(&_par.repeat_count)->default_value(10), "Per message trip count")("message-count", value<uint32_t>(&_par.message_count)->default_value(1000), "Message count")("min-size", value<uint32_t>(&_par.min_size)->default_value(10), "Min message data size")("max-size", value<uint32_t>(&_par.max_size)->default_value(500000), "Max message data size")("netid,n", value<uint32_t>(&_par.netid), "Network identifier");
        variables_map vm;
