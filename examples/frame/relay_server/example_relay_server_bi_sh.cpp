...
class BufferBase{
    ...
    size_t  bc_;
protected:
    BufferBase(char *_pb, size_t _bc): pb_(_pb), bc_(_bc){}
public:
...
public:
    Buffer(size_t _cp):BufferBase(new char[_cp], C){}
};
...
class Buffer: public BufferBase{
    char b[C];
public:
    Buffer(size_t _cp = 0):BufferBase(b, C){}
};

constexpr size_t buffer_capacity(const size_t _sz){
    if(_sz <= 512){
        return 512;
    }else if(_sz <= 1024){
        return 1024;
...
struct EventData;
using EventDataPtrT = std::shared_ptr<EventData>;
    ...
    do {
        if (!_rctx.error()) {
...

            {
                ...
                rsch_.startObject(objptr, rsvc, make_event(GenericEvents::Start), err);
            }
        } else {
...
/*virtual*/ void Connection::onEvent(frame::aio::ReactorContext& _rctx, Event&& _revent)
{
    ...
    if (generic_event_raise == _revent) {
        ...
        if (evtdataptr->bufsz_) {
            ...
            if (rbufptr_) {
                revtdataptr->bufptr_->next_ = std::move(rbufptr_->next_);
                rbufptr_->next_ = std::move(revtdataptr->bufptr_);
            }
            else {
                rbufptr_ = std::move(revtdataptr->bufptr_);
                sock_.postRecvSome(_rctx, rbufptr_->data(), rbufptr_->capacity(), Connection::onRecv);
            }
            //then store the event data ptr
            revtdataptr->nextptr_ = std::move(reventdataptr_);
            reventdataptr_ = std::move(revtdataptr);
        }
...
/*static*/ void Connection::onRecv(frame::aio::ReactorContext& _rctx, size_t _sz)
{
    ...
    if (!_rctx.error()) {
        ...
        _rctx.manager().notify(rthis.peer_objuid_, make_event(GenericEvents::Raise, makeReadEventData(_sz)));

       if (prepareCurrentReadBuffer()) {
            rthis.sock_.postRecvSome(_rctx, rthis.rbufptr_->data(), rthis.rbufptr_->capacity(), Connection::onRecv);
        } else {
            //no free buffer
        }
...
/*static*/ void Connection::onSend(frame::aio::ReactorContext& _rctx)
{
    ...
    if (!_rctx.error()) {
        _rctx.manager().notify(rthis.peer_objuid, make_event(GenericEvents::Raise, popWriteEventData()));

        idbg(&rthis << " ");
        
        if (rthis.weventdatabegptr_) {
            rthis.sock_.postSendAll(_rctx, weventdatabegptr_->bufptr_->data(), weventdatabegptr_->bufsz_, Connection::onSend);
        }
