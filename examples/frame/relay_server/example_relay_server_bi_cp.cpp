...
struct ResolvFunc {
    ...
    void operator()(ResolveData& _rrd, ErrorCodeT const& _rerr)
    {
        ...
        rm.notify(actuid, make_event(GenericEventE::Message, _rrd));
    }
};

size_t Connection::doneBuffer(frame::aio::ReactorContext& _rctx)
{
    _rctx.manager().notify(peer_actuid, make_event(GenericEventE::Wake));
...
/*static*/ void Connection::onConnect(frame::aio::ReactorContext& _rctx)
{
    ...
    if (!_rctx.error()) {
        ...
        auto ev(make_event(GenericEventE::Message, _rctx.manager().id(rthis)));
