...
*/
class Reader: public solid::protocol::Reader{
public:
	enum{
		QuotedString = LastBasicError
	};
	Reader(Writer &_rw, solid::protocol::Logger *_plog = NULL);
	~Reader();
	void clear();
	//! Asynchrounously reads an astring (atom/quoted/literal)
	static int fetchAString(solid::protocol::Reader &_rr, solid::protocol::Parameter &_rp);
	//! Asynchrounously reads an qstring
	static int fetchQString(solid::protocol::Reader &_rr, solid::protocol::Parameter &_rp);
	//! Asynchrounously flushes the writer
	static int flushWriter(solid::protocol::Reader &_rr, solid::protocol::Parameter &_rp);
	//! Asynchrounously copies the temp string to a given location
	static int copyTmpString(solid::protocol::Reader &_rr, solid::protocol::Parameter &_rp);
	//! Asynchrounously check for the type of the literal (plus or not)
	static int checkLiteral(solid::protocol::Reader &_rr, solid::protocol::Parameter &_rp);
	//! Asynchrounously prepares for error recovery
	static int errorPrepare(solid::protocol::Reader &_rr, solid::protocol::Parameter &_rp);
	//! Asynchrounously recovers from an error
	static int errorRecovery(solid::protocol::Reader &_rr, solid::protocol::Parameter &_rp);
	//! Local asynchrounous reinit method.
	template <class C>
	static int reinit(solid::protocol::Reader &_rr, solid::protocol::Parameter &_rp){
		return reinterpret_cast<C*>(_rp.a.p)->reinitReader(static_cast<Reader&>(_rr), _rp);
	}
	template <class C, int V>
	static int reinitExtended(solid::protocol::Reader &_rr, solid::protocol::Parameter &_rp){
