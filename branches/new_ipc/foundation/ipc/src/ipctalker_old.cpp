...
struct Talker::Data{
	...
	struct SignalData{
		SignalData(
			...
			uint16 _procuid,
			uint32 _flags
		):	psig(_psig), procid(_procid),
			procuid(_procuid), flags(_flags){}
	...
	struct SendBufferDataCmp{
		bool operator()(
			const SendBufferData* const &_rpon1,
			const SendBufferData* const &_rpon2
		)const{
	...
	Data(
		Service &_rservice,
		uint16 _id
	):rservice(_rservice), nextprocid(0), tkrid(_id){}
...
int Talker::pushSignal(DynamicPointer<Signal> &_psig, const ConnectionUid &_rconid, uint32 _flags){
...
void Talker::pushProcessConnector(ProcessConnector *_pc, ConnectionUid &_rconid, bool _exists){
	...
	switch(d.rcvbuf.type()){
		...
		case Buffer::DataType:{
			...
			if(pit != d.peerpm4.end()){
				...
				cassert(rpp.first);
				ConnectionUid conid(d.tkrid, pit->second, rpp.second);
