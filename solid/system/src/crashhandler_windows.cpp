...
LONG WINAPI exceptionHandling(EXCEPTION_POINTERS* info, const std::string& handler)
{
    ...
    const auto fatal_id = static_cast<solid::SignalType>(exception_code);
    std::cerr << fatal_stream.str() << " " << dump << std::endl;
...
{
    const solid::SignalType exception_code = p->ExceptionRecord->ExceptionCode;
...
void exit_with_default_signal_handler(solid::SignalType fatal_signal_id)
{
...
{
    install_signal_handler_for_thread();
}
...
{
    internal::install_signal_handler();
