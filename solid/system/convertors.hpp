...
{
    return (static_cast<uint32_t>(1) << v) - 1;
}
constexpr inline uint32_t bits_to_count32(unsigned v)
{
    return (static_cast<uint32_t>(1) << v) & (~static_cast<uint32_t>(1));
}

constexpr inline uint64_t bits_to_mask64(unsigned v)
{
    return (static_cast<uint64_t>(1) << v) - 1;
}

constexpr inline uint64_t bits_to_count64(unsigned v)
{
    return (static_cast<uint64_t>(1) << v) & (~static_cast<uint64_t>(1));
}
...
{
    return (static_cast<size_t>(1) << v) & (~static_cast<size_t>(1));
}
