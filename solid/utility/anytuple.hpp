...
public:
    AnyTuple(const std::tuple<Args...> &_tuple):tuple_(_tuple){}
    AnyTuple(std::tuple<Args...> &&_tuple):tuple_(std::forward<std::tuple<Args...>>(_tuple)){}
};
...
public:
    ...
    template <typename ...Args>
    AnyTuple(std::tuple<Args...> &&_tuple):base_ptr_(std::make_unique<detail::AnyTuple<Args...>>(std::forward<std::tuple<Args...>>(_tuple))){
    ...
    AnyTuple(AnyTuple &&_other): base_ptr_(std::move(_other.base_ptr_)){
    }
