...
TestBase* create_test(const FunctionChoice _fnc_choice, const size_t _closure_size){
    switch(_fnc_choice){
        case FunctionChoice::Solid:
            return create_test<solid::Function<32, uint64_t(const size_t)>>(_closure_size);
...
int test_function_perf(int argc, char *argv[]){
    ...
    if(argc > 1){
        switch(argv[1][0]){
            case 's': fnc_choice = FunctionChoice::Solid; break;
            case 'S': fnc_choice = FunctionChoice::Standard; break;
            default:
