    ...
    Context& _rctx)
{
    ...
    if (!mprpcclient_ptr) { // mprpc client initialization
        ...
        err = mprpcclient_ptr->sendMessage(
            "localhost", frame::mprpc::make_message<beta_protocol::FirstMessage>(100000, make_string(100000)),
            {frame::mprpc::MessageFlagsE::AwaitResponse});
        if (err) {
            return err;
        }
        err = mprpcclient_ptr->sendMessage(
            "localhost", frame::mprpc::make_message<beta_protocol::SecondMessage>(200000, make_string(200000)),
            {frame::mprpc::MessageFlagsE::AwaitResponse});
        if (err) {
            return err;
        }
        err = mprpcclient_ptr->sendMessage(
            "localhost", frame::mprpc::make_message<beta_protocol::ThirdMessage>(30000, make_string(30000)),
            {frame::mprpc::MessageFlagsE::AwaitResponse});
        if (err) {
            return err;
        }
