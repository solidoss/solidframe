    ...
    if (_rrecv_msg_ptr) {
        ...
        if (crtwriteidx < writecount and pmprpcclient) {
            err = pmprpcclient->sendMessage(
                client_id, frame::mprpc::make_message<Message>(crtwriteidx++),
...
int test_pool_basic(int argc, char* argv[])
{
...

    {
        ...
        { // mprpc client initialization
            ...
            cfg.client.name_resolve_fnc.emplace(frame::mprpc::InternetResolverF(resolver, server_port /*, SocketInfo::Inet4*/));
