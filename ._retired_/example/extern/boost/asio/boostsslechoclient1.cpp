...
public:
    session(
        ...
        Manager &_rm, uint32_t _idx
    )
        :   rm(_rm), socket_(io_service, context), mintime(0xffffffff),
            maxtime(0), crtread(0), readcnt(0), readsz(0),
            writecnt(0), writesz(0), writecrt(0), usecnt(0), writeoff(0), idx(_idx)
    {
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        int idx = 0;
        while(concnt--){
            session *ps = new session(io_service, ctx, m, idx);
            ps->start(m.endPoint());
            ++idx;
        }
        io_service.run();
